...
class OR_solver_TW(object):
    def __init__(self,fin='csv/distance_matrix.csv'):
    ...
    def print_solution(self, manager, routing, assignment):
        ...
        total_time = 0
        for vehicle_id in range(self.data['num_agents']):
            index = routing.Start(vehicle_id)
            plan_output = 'Route for agent {}:\n'.format(vehicle_id)
    ...
    def run(self):
        ...
        manager = pywrapcp.RoutingIndexManager(
            len(self.data['time_matrix']), self.data['num_agents'], self.data['depot'])
        ...
        for i in range(self.data['num_agents']):
